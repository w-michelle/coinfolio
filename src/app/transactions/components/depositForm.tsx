...
import { addMoney } from "@/server/actions/addMoney";
...
export const DepositForm = ({ data }: { data: PortfolioProps }) => {
  ...
  const [state, execute, isPending] = useActionState(addMoney, null);

  const onSubmit = (fdata: z.infer<typeof formSchema>) => {
    const formData = new FormData();
    formData.append("amount", fdata.amount.toString());
    startTransition(() => {
      execute(formData);
      reset();
    });
  ...
  return (
    ...
    >
      <h3 className="text-neutral-700 font-bold tracking-wider">Deposit</h3>
      <div>
